    ...
    if workers == 1:
        for k_ in range(k):
            ...
            if need_v:
                ...
                for g_ in range(3):
                    v[:, :, g_, k_] = np.maximum(0, 1 + fn * cs[0, g_] + tp * cs[1, g_] + fp * cs[2, g_])
...
def edges_eval_dir(res_dir, gt_dir, cleanup=0, thrs=99, max_dist=0.0075, thin=True, workers=1):
    ...
    for i, name in enumerate(ids):
        ...
        res = np.loadtxt(os.path.join(eval_dir, "{}_ev1.txt".format(name)), dtype=np.float32)
        t_i, res = res[:, 0], res[:, 1:]
        cnt_sum_r_p += res
        r, p, f = compute_rpf(res)
        k = f.argmax()
        ois_r1, ois_p1, ois_f1, ois_t1 = find_best_rpf(t_i, r, p)
    ...
    r_i, p_i, t_i, f_i, ap = r[k], p[k], t[k], f[k], 0
    if len(r_i) > 1:
        ap = interp1d(r_i, p_i, bounds_error=False, fill_value=0)(np.linspace(0, 1, 101))
        ap = np.sum(ap) / 100.0
    _, o = np.unique(p_i, return_index=True)
    r50 = interp1d(p_i[o], r_i[o], bounds_error=False, fill_value=np.nan)(np.maximum(p_i[o[0]], 0.5))

    bdry = np.array([[ods_t, ods_r, ods_p, ods_f, ois_r.item(), ois_p.item(), ois_f.item(), ap]])
    bdry_thr = np.stack([t_i, r_i, p_i, f_i], axis=0).T
